...
shell:
	docker compose exec -it provision-provider-website-builders /bin/bash

# Run Static Analysis (PHPStan)
static-analysis:
	docker compose exec provision-provider-website-builders ./vendor/bin/phpstan analyse --memory-limit=1G

coding-standards:
	docker compose exec provision-provider-website-builders php ./bin/php-cs-fixer-v3.phar fix --config=./.php-cs-fixer.dist.php
...
composer-install:
	docker compose exec provision-provider-website-builders composer install --no-interaction
...
--remove-lockfile:
	docker compose exec provision-provider-website-builders rm -f ./composer.lock

# Remove vendor directory
--remove-vendor:
	docker compose exec provision-provider-website-builders rm -rf ./vendor
